        ...
        if(org != null && org.organisation_id > 0) {
            ...
            if(findRepresentative != null && findRepresentative.length > 0) {
                ...
                assignmentsList =  await connection.resources.query(queryAllAssignments,{
                    type: connection.Sequelize.QueryTypes.SELECT,
                    replacements: { year: 1997, organisationID: org.organisation_id  },
                    raw: true,
                    logging: console.log,
                    }
                );
    ...
    if(parseInt(customerID) > 0) {
        ...
        if(org != null && org.organisation_id > 0) {
            ...
            if(representativeIDs != null && representativeIDs.length > 0) {
                ...
                const queryAllConveyance = "SELECT ac.convey_ty as name FROM assignment_conveyance AS ac WHERE ac.rf_id IN (SELECT d.rf_id FROM documentid as d WHERE appno_doc_num IN (SELECT appno_doc_num FROM documentid WHERE appno_doc_num <> '' AND rf_id IN (SELECT rf_id FROM list2 WHERE organisation_id = :organisationID AND company_id IN (:representativeID)) GROUP BY appno_doc_num ) GROUP BY d.rf_id) GROUP BY ac.convey_ty";
...
let getCompaniesWithChildren = async (DBConnection, organisationID) => {
    let companies = [];
    const parentCompanyQuery = "SELECT representative_id as id, '' AS slack, original_name, representative_name, instances, instances + (Select sum(instances) FROM representative as r1 WHERE r1.parent_id = r.representative_id) as counter, type FROM representative as r WHERE r.parent_id = 0";
    ...
    if(companies.length > 0) {
        ...
        let childCompaniesQuery = "SELECT representative_id as id, '' AS slack, original_name, representative_name, instances as counter, parent_id FROM representative as r WHERE r.parent_id IN (:parentCompany) AND child = :child ORDER BY r.parent_id ASC, counter DESC";

...
const reelFrameSchema = z.object({
  params: z.object({ reel_frame: z.string().regex(/^\d+-\d+$/, 'expected reel-frame, e.g. 45231-0812') }),
});
const applicationSchema = z.object({
  params: z.object({ application_number: z.string().min(1).max(50) }),
});
...
router.get('/connection/asset/:application_number', [verifyToken, attachTenant], validate(applicationSchema), controller.byApplication);
router.get('/connection/:reel_frame', verifyToken, validate(reelFrameSchema), controller.byReelFrame);

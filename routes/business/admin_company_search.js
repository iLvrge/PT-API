...
const Correspondence = require('../../model/resources/Correspondence');
    ...
    try{
        ...
        if( findAllLenders.length > 0 ) {
            ...
            if( firmIDs.length > 0 ) {
                ...
                WHERE (ass.assignor_and_assignee_id = a.assignor_and_assignee_id OR aaa.assignor_and_assignee_id = a.assignor_and_assignee_id) AND docID.appno_doc_num =  doc.appno_doc_num  GROUP BY docID.appno_doc_num) AS owned,
                (
                    SELECT docID1.appno_doc_num FROM assignor AS aor
                                ...
                                LEFT JOIN representative as r ON r.representative_id = aaa.representative_id
                                WHERE (aor.assignor_and_assignee_id = a.assignor_and_assignee_id OR aaa.assignor_and_assignee_id = a.assignor_and_assignee_id) AND docID1.appno_doc_num =  doc.appno_doc_num
...
route.get("/company/:companyID/law_firms", [authJWT.verifyToken, authJWT.isAdmin], async (req, res, next) => {
    try {        
        ...
        LEFT JOIN db_uspto.representative_law_firm AS representative_law_firm ON representative_law_firm.representative_id =  law_firms.representative_id 
                ...
                INNER JOIN assignee ON assignee.rf_id = cor.rf_id 
        ...
        if(rf_id > 0) {
            ...
            if(getData != null && getData.rf_id > 0) {
                ...
                if(getData.law_firm_id > 0) {
                    ...
                    const findLawFirmData = await LawFirms.findOne({where: {law_firm_id: getData.law_firm_id}})

                    if(findLawFirmData != null) {
                        lawfirmID = getData.law_firm_id;
                        if(findLawFirmData.name != caddress_1) {
                            const checkLawFirm = await LawFirms.findOne({where:{name: caddress_1}})
                            if(checkLawFirm == null) {
                                findLawFirmData.name = caddress_1
                                findLawFirmData.save()
                            } else {
                                ...
                                lawfirmID = checkLawFirm.law_firm_id;
                ...
                /* if(lawfirmID == 0) {
                    ...
                    if(preCAddress1 != caddress_1) {
                        const findLawFirm = await LawFirms.findOne({where:{name: preCAddress1}})
                        if(findLawFirm != null) {
                            await LawFirms.destroy({where: {law_firm_id: findLawFirm.law_firm_id}});
                            await Lawyers.destroy({where: {law_firm_id: findLawFirm.law_firm_id}});
                        }
                    } 
                } */
                ...
                /* if(lawfirmID > 0) {
                    let layerID = 0
                    if(cname != '') { 
                        let getLawyer = await Lawyers.findOne({
                            where: {
                        ...
                        if(getLawyer == null) {
                            getLawyer = await Lawyers.findOne({
                                where: {
                                    law_firm_id: lawfirmID,
                                    name: preCName
                                }

        ...
        if(typeof req.connection_db != "undefined" && req.connection_db != null ) {
            ...
            if(getAllNames.length > 0) {
                ...
                if( allNames.length > 0 ) {
                    ...
                    const customQuery = "SELECT STRING_REPLACE FROM db_patent_maintainence_fee.event_maintainence_fees WHERE event_code NOT IN (:eventCode) AND appno_doc_num IN ( SELECT appno_doc_num FROM documentid WHERE grant_doc_num <> '' AND date_format(grant_date, '%Y-%m-%d') BETWEEN :startDate AND :endDate AND rf_id IN ( SELECT ee.rf_id  from assignee as ee INNER JOIN assignment_conveyance as ac ON ac.rf_id = ee.rf_id LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = ee.assignor_and_assignee_id WHERE aaa.name IN (:representativeNames) AND ac.convey_ty IN (:activityType)  AND  ac.rf_id IN (SELECT rf_id FROM db_uspto.representative_transactions WHERE representative_id IN (:representativeIds) AND organisation_id = :organisationID ))  AND appno_doc_num NOT IN ( SELECT appno_doc_num FROM documentid WHERE rf_id IN (SELECT ass.rf_id FROM assignor as ass  INNER JOIN assignment_conveyance as ac ON ac.rf_id = ass.rf_id LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = ass.assignor_and_assignee_id WHERE aaa.name IN (:representativeNames) AND ac.convey_ty IN (:activityType) AND ac.rf_id IN (SELECT rf_id FROM db_uspto.representative_transactions WHERE representative_id IN (:representativeIds) AND organisation_id = :organisationID )) GROUP BY appno_doc_num ) GROUP BY appno_doc_num ) AND appno_doc_num NOT IN (SELECT appno_doc_num FROM db_patent_maintainence_fee.event_maintainence_fees WHERE event_code IN (:eventCode) AND appno_doc_num IN ( SELECT appno_doc_num FROM documentid WHERE grant_doc_num <> '' AND date_format(grant_date, '%Y-%m-%d') BETWEEN :startDate AND :endDate AND rf_id IN ( SELECT ee.rf_id  from assignee as ee INNER JOIN assignment_conveyance as ac ON ac.rf_id = ee.rf_id LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = ee.assignor_and_assignee_id WHERE aaa.name IN (:representativeNames) AND ac.convey_ty IN (:activityType)  AND  ac.rf_id IN (SELECT rf_id FROM db_uspto.representative_transactions WHERE representative_id IN (:representativeIds) AND organisation_id = :organisationID ))  AND appno_doc_num NOT IN ( SELECT appno_doc_num FROM documentid WHERE rf_id IN (SELECT ass.rf_id FROM assignor as ass  INNER JOIN assignment_conveyance as ac ON ac.rf_id = ass.rf_id LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = ass.assignor_and_assignee_id WHERE aaa.name IN (:representativeNames) AND ac.convey_ty IN (:activityType) AND ac.rf_id IN (SELECT rf_id FROM db_uspto.representative_transactions WHERE representative_id IN (:representativeIds) AND organisation_id = :organisationID )) GROUP BY appno_doc_num ) GROUP BY appno_doc_num )  GROUP BY appno_doc_num)  GROUP BY appno_doc_num ";
                    ...
                    */
                    ...
                    currentDate = new Date()
                    const earlierEndDate = moment( currentDate.setFullYear( currentDate.getFullYear() - 3 )).format('YYYY-MM-DD'), earlierEventCode = ['M1551', 'M2551', 'M3551','EXP.', 'EXPX']
                    let replacements = { representativeIds: representativeIDs, organisationID: req.orgId, representativeNames: allNames, activityType };

                    let listQuery = `SELECT CASE WHEN grant_doc_num = '' THEN appno_doc_num ELSE grant_doc_num END AS asset, CASE WHEN grant_doc_num = '' THEN 1 ELSE 0 END AS asset_type, '' AS channel, appno_doc_num, grant_doc_num, grant_date,  date_format(DATE_ADD(grant_date, INTERVAL 36 MONTH), "%b %d, %Y") as payment_due, date_format(DATE_ADD(grant_date, INTERVAL 42 MONTH), "%b %d, %Y") as payment_grace, 1 as type, "1551" as fee_code, 2000 as fee_amount, 1554 as fee_code_surcharge, 500 as fee_surcharge, "" as remaining_year, "" as source, "" as fwd_citation, "" as technology, 0 as child_count FROM documentid WHERE appno_doc_num IN (${customQuery.replace('STRING_REPLACE', ' appno_doc_num')}) AND appno_doc_num NOT IN (SELECT appno_doc_num FROM assets_transfer WHERE appno_doc_num <> '' AND status = 0 AND layout_id = 3) AND grant_doc_num NOT IN (SELECT grant_doc_num FROM assets_transfer WHERE appno_doc_num = '' AND grant_doc_num <> '' AND status = 0 AND layout_id = 3) GROUP BY grant_doc_num`
                    ...
                    const midStartDate = moment( currentDate.setFullYear( currentDate.getFullYear() - 8 ) ).format('YYYY-MM-DD')
                    currentDate = new Date()
                    const midEndDate =  moment( currentDate.setFullYear( currentDate.getFullYear() - 7 ) ).format('YYYY-MM-DD'), midEventCode = ['M1552', 'M2552', 'M3552','EXP.', 'EXPX']

                    listQuery = `SELECT CASE WHEN grant_doc_num = '' THEN appno_doc_num ELSE grant_doc_num END AS asset, CASE WHEN grant_doc_num = '' THEN 1 ELSE 0 END AS asset_type, '' AS channel,  appno_doc_num, grant_doc_num, grant_date,  date_format(DATE_ADD(grant_date, INTERVAL 84 MONTH), "%b %d, %Y") as payment_due, date_format(DATE_ADD(grant_date, INTERVAL 90 MONTH), "%b %d, %Y") as payment_grace, 2 as type, "1552" as fee_code, 3760 as fee_amount, 1555 as fee_code_surcharge, 500 as fee_surcharge, "" as remaining_year, "" as source, "" as fwd_citation, "" as technology, 0 as child_count FROM documentid WHERE appno_doc_num IN (${customQuery.replace('STRING_REPLACE', ' appno_doc_num')})AND appno_doc_num NOT IN (SELECT appno_doc_num FROM assets_transfer WHERE appno_doc_num <> '' AND status = 0 AND layout_id = 3) AND grant_doc_num NOT IN (SELECT grant_doc_num FROM assets_transfer WHERE appno_doc_num = '' AND grant_doc_num <> '' AND status = 0 AND layout_id = 3) GROUP BY grant_doc_num`
                    getList = await connection.application.query( listQuery, {
                            type: connection.Sequelize.QueryTypes.SELECT,
                            raw: true,
                            logging: console.log,
                    ...
                    currentDate = new Date()
                    const finalEndDate = moment( currentDate.setFullYear( currentDate.getFullYear() - 11 )).format('YYYY-MM-DD'), finalEventCode = ['M1553', 'M2553', 'M3553','EXP.', 'EXPX']         

                    listQuery = `SELECT CASE WHEN grant_doc_num = '' THEN appno_doc_num ELSE grant_doc_num END AS asset, CASE WHEN grant_doc_num = '' THEN 1 ELSE 0 END AS asset_type, '' AS channel,  appno_doc_num, grant_doc_num, grant_date, date_format(DATE_ADD(grant_date, INTERVAL 132 MONTH), "%b %d, %Y") as payment_due, date_format(DATE_ADD(grant_date, INTERVAL 138 MONTH), "%b %d, %Y") as payment_grace, 3 as type, "1553" as fee_code, 7700 as fee_amount, 1556 as fee_code_surcharge, 500 as fee_surcharge, "" as remaining_year, "" as source, "" as fwd_citation, "" as technology, 0 as child_count FROM documentid WHERE appno_doc_num IN (${customQuery.replace('STRING_REPLACE', ' appno_doc_num')}) AND appno_doc_num NOT IN (SELECT appno_doc_num FROM assets_transfer WHERE appno_doc_num <> '' AND status = 0 AND layout_id = 3) AND grant_doc_num NOT IN (SELECT grant_doc_num FROM assets_transfer WHERE appno_doc_num = '' AND grant_doc_num <> '' AND status = 0 AND layout_id = 3) GROUP BY grant_doc_num`

...
route.post("/assets/cpc", [authJWT.verifyToken, clientDBConnection.connect], async (req, res, next) => {
    try {
        ...
        if (typeof req.shareCode != 'undefined' && req.shareCode != '' && companies.length == 0) {
...
route.get("/assets/:patentNumber/:type/outsource", [], async (req, res) => {
    try {
        const { patentNumber, type } = req.params;
        const flag = req.query.flag;

        if (type === "1") {
            let queryType = "patentNumber";
            let where = {
                [connection.Op.or]: [{ grant_doc_num: patentNumber }, { appno_doc_num: patentNumber }]
            };

            if (typeof flag !== 'undefined' && flag >= 0) {
                queryType = flag === 1 ? "patentNumber" : "applicationNumber";
                where = flag === 1 
            ...
            if (!record) {
                record = await connection.resources.query(
                    `SELECT grant_doc_num from db_patent_application_bibliographic.application_grant WHERE grant_doc_num = :number`,
                    {
                        type: connection.Sequelize.QueryTypes.SELECT,
            ...
            if (!record) {
                record = await connection.resources.query(
                    `SELECT appno_doc_num from db_patent_grant_bibliographic.application_publication WHERE grant_doc_num = :number`,
                    {
                        type: connection.Sequelize.QueryTypes.SELECT,
                        replacements: { number: patentNumber },
                        raw: true,
            ...
            if (record) {
                const assetURL = `${process.env.ASSIGNMENT_CENTER_SEARCH_URL}abstract%3F${queryType}%3D${patentNumber}`;
                res.status(200).json({ url: assetURL });
            } else {
                res.status(200).send("");
            }
        } else if (type === "0") {
            const assignment = await ResourceAssignments.findOne({
                where: { rf_id: patentNumber },
                attributes: ['reel_no', 'frame_no']
            });

            if (assignment) {
                const frame = assignment.frame_no.toString().padStart(4, '0');
                const searchInput = `${assignment.reel_no}-${frame}`;
                const transactionURL = `${process.env.ASSIGNMENT_CENTER_SEARCH_URL}reelFrameDetail%3FreelFrame%3D${searchInput}`;
                res.status(200).json({ url: transactionURL });
            } else {
                res.status(200).send("");
            }
        }
    } catch (err) {
        console.error("Outsourcing error:", err);
    ...
    if (typeof req.connection_db != "undefined" && req.connection_db != null) {
        ...
        if (findRFIDs != null) {
            ...
            assets = await Documentids.findAll({
                ...
                where: {
                    rf_id: allRFIDs, [connection.Op.or]: [
                        { appno_doc_num: query },
                        { grant_doc_num: query },
                    ]
                }

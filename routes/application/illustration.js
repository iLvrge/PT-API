...
let createJSON = async(itemDetails, rfID) => {
    ...
    if(itemDetails !== undefined && itemDetails.assignor.length > 0) {
        ...
        itemDetails.assignor.forEach( (assignor, index) => {
            ...
            assignors.push({recorded_name: assignor.original_name, normalize_name: assignor.representative_original_name});
        ...
        itemDetails.assignee.forEach( assignee => {
            ...
            assignees.push({recorded_name: assignee.original_name, normalize_name: assignee.representative_original_name});
...
route.get('/connection/:reelFrame',  /* [authJWT.verifyToken], */ async (req, res) => {  
    ...
    if( reelFrame != '' && reelFrame != null && reelFrame != undefined ) {
        reelFrame = reelFrame.split('-')
        if( reelFrame.length == 2 ) {
            const findRfID = await Assignments.findOne({
                attributes: ['rf_id'],
                where: { reel_no: reelFrame[0] , frame_no: reelFrame[1] }
            })
        
            if( findRfID && findRfID != null) {
                const itemDetails = await helpers.getAssignmentDataByrfID(findRfID.rf_id);
                await createJSON(itemDetails)
                const illustrationData = await createJSON(itemDetails, findRfID.rf_id)        
...
route.get("/collections/:rf_id/illustration", [authJWT.verifyToken], async (req, res) =>{     
    ...
    if(rfID > 0) {
        ...
        const illustrationData = await createJSON(itemDetails, rfID.toString().trim())        
        res.status(200).json( illustrationData );		
    } else {
